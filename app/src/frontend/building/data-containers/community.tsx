...
const CommunityView: React.FunctionComponent<CategoryViewProps> = (props) => {
   ...
   const switchToExpectedApplicationMapStyle = (e) => {
    ...
    return (
        <Fragment>
            <DataEntryGroup name={t("Community views on how well buildings work")}>
            <div className={`alert alert-dark`} role="alert" style={{ fontSize: 13, backgroundColor: "#f6f8f9" }}>
                <i>
                    {t("Note: We are currently only collecting data on non-residential buildings.")}
                </i>
            </div>
            <div className='community-opinion-pane'>
                ...
                {(props.mapColourScale == "typology_likes") ? 
                    <button className={`map-switcher-inline enabled-state btn btn-outline btn-outline-dark ${darkLightTheme}`} onClick={switchToLocalSignificanceMapStyle}>
                        {t('Click here to change map to buildings of local interest.')}
                    </button>
                :
                    <button className={`map-switcher-inline disabled-state btn btn-outline btn-outline-dark ${darkLightTheme}`} onClick={switchToTypologyMapStyle}>
                        {t("Click to return to liked typologies mapped.")}
                    </button>
                }
                {
                ...
                <div className={`alert alert-dark`} role="alert" style={{ fontSize: 13, backgroundColor: "#f6f8f9" }}>
                    <i>
                        {t("For more information on current planning applications, refer to the Planning Controls category.")}
                    </i>
                </div>
                ...
                :
                    <button className={`map-switcher-inline disabled-state btn btn-outline btn-outline-dark ${darkLightTheme}`} onClick={switchToExpectedApplicationMapStyle}>
                        {t("Click here to change map to planning applications expected by community.")}
                    </button>
                }
            </div>
            </DataEntryGroup>
            <DataEntryGroup name={t("Buildings in community use")}>
            <div className={`alert alert-dark`} role="alert" style={{ fontSize: 13, backgroundColor: "#f6f8f9" }}>
                <i>
                    {t("Here we are collecting information on the location of buildings used for community activities so we can track loss of/additions to community space over time.")}
                </i>
            </div>
            <LogicalDataEntry
                slug='community_activities_current'
                title={t(dataFields.community_activities_current.title)}
                tooltip={t(dataFields.community_activities_current.tooltip)}
            ...
            <LogicalDataEntry
                ...
                tooltip={t(dataFields.community_activities.tooltip)}
            ...
            <LogicalDataEntry
                ...
                tooltip={t(dataFields.community_activities_always.tooltip)}
            ...
            :
                <button className={`map-switcher-inline disabled-state btn btn-outline btn-outline-dark ${darkLightTheme}`} onClick={switchToPublicOwnershipMapStyle}>
                    {t("Click here to see ownership type mapped.")}
                </button>
            }
            </DataEntryGroup>
        </Fragment>
